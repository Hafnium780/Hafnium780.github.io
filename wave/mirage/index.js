...
const createCanvas = () => {
  ...
  for (let i = 0; i < heightCells; i++) {
    ...
    for (let j = 0; j < widthCells; j++) {
      ...
      else n[i][j] = 1 / Math.sqrt(i / heightCells);
  ...
  for (let s = 0; s < skip; s++) {
    ...
    for (let i = 0; i < heightCells; i++) {
      ...
      for (let j = 0; j < widthCells; j++) {
        ...
        if (i > (heightCells * 7) / 8) newGrid[i][j] *= 0.99;
        if (i < heightCells / 8) newGrid[i][j] *= 0.98;
        if (j > (widthCells * 7) / 8) newGrid[i][j] *= 0.99;
      }
    }
    for (let i = 0; i < heightCells; i++) {
      for (let j = 0; j < widthCells; j++) {
    ...
    for (let i = Math.floor(heightCells / 8); i < heightCells / 4; i++) {
      grid[i][0] = m * Math.sin(w * t - (i * 3) / 4);
    }
