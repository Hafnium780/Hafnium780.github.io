...
let colorScale = 255 / 20; // color units / magnitude units
...
document.addEventListener("mousemove", (e) => {
  const newX = Math.floor(e.clientX / resolution);
  const newY = Math.floor(e.clientY / resolution);
  if (prevX) {
    const dX = newX - prevX;
    const dY = newY - prevY;
    const stepCount = Math.max(Math.abs(dX), Math.abs(dY));
    for (let s = 0; s < stepCount; s++) {
      grid[Math.floor(prevY + (s / stepCount) * dY)][
        Math.floor(prevX + (s / stepCount) * dX)
      ] = 50;
    }
  }
