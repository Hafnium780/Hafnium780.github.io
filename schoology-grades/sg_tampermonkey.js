...
#title { color: #F5F4F3; font-size: 25px; margin-bottom: 6px; font-weight: 700; }
...
.course-grade-grade { width: 12%; text-align: center; padding: 5px; background-color: #E9BE78; border-radius: 10px; margin-left: 16px; }
...
#assignment-grades { overflow: auto; width: calc(100vw - 450px); height: calc(100vh - 65px); background-color: #1E1F21; }
...
.categorytitlediv { border-top: 2px solid #424244; position: sticky; background-color: #1E1F21;
                    top: 0px; display: flex; flex-direction: row; margin-bottom: 3px; padding: 5px; color: #F5F4F3; }
.categorypoints { font-size: small; flex: 0 0; text-align: center; padding: 5px; background-color: #E9384F; border-radius: 10px; min-width: 100px; margin-right: 10px; }
...
.categoryweight { font-size: small; padding: 5px; text-align: center; background-color: #E9384F; border-radius: 10px; margin-left: 5px; flex: 0 0; min-width: 100px; margin-right: 10px; }
.categoryweightmethod { cursor: pointer; font-size: small; padding: 5px; text-align: center; background-color: #AC143C; border-radius: 10px; margin-left: 5px; flex: 0 0; min-width: 100px; margin-right: calc(100% - 434px); transition: background-color ease 200ms; }
...
.assignment-grade-grade { font-size: small; text-align: right; flex-grow: 0; flex-shrink: 0; vertical-align: middle; text-align: center; padding-right: 10px; padding: 5px; background-color: #E9BE78; border-radius: 10px; width: 60px; }
...
.reloadname { border-radius: 7px; width: 15px; height: 15px; font-size: 12px; background-color: gray; flex: 0 0 15px; left: 2px; text-align: center; color: white; margin: 0px 2px; cursor: pointer; }
...
<div id="grades">
  ...
  <div id="assignment-grades">
    ...
    let grades, courses, gCount, assignments, assignmentNames, assignmentFactors, categoryTypes;
    ...
    const getContent = (a, xml=false) => {
        return new Promise((res, rej) => {
            let link = a.indexOf(protocol + "://api.schoology.com") === -1
    ...
    const displayAssignments = (s, id) => {
        ...
        let calculated_grade_title = document.createElement("div");
        let calculated_grade = document.createElement("div");
        let calculated_grade_error = document.createElement("div");
        ...
        calculated_grade_error.innerText = "Not matching with Schoology? Try switching the weighting method (Percentage or Points) for individual categories.";
        assignment_grades.appendChild(calculated_grade_title);
        assignment_grades.appendChild(calculated_grade);
        assignment_grades.appendChild(calculated_grade_error);
        const categories = [];
        ...
        for (let i = 0; i < s.grading_category.length; i++) {
            ...
            if (totalWeight === 100) {
                if (s.grading_category[i].weight) weight = s.grading_category[i].weight / totalWeight;
                else weight = 0;
            }
            ...
            methoddiv.innerText = (categoryTypes[gc.id] ?? 2) == 1 ? "Percentage" : "Points";

            let id = gc.id;
            methoddiv.addEventListener("click", () => {
                categoryTypes[id] = (categoryTypes[gc.id] ?? 2) == 1 ? 2 : 1;
                methoddiv.innerText = categoryTypes[gc.id] == 1 ? "Percentage" : "Points";
        ...
        for (let i = 0; i < assignments.length; i++) {
            ...
            reloadName.classList.add("reloadname");
            reloadName.innerText = String.fromCodePoint(8635); // ↻
            grade.innerText = a.grade;
            if (a.assignmentName == undefined || a.assignmentName == "le>") assignmentName.innerText = "unknown";
...
//        }
    ...
    const updateGrades = (s, categories) => {
        ...
        for (let i = 0; i < assignments.length; i++) {
            ...
            if (a.rawGrade.pts !== null) {
                if (cat.calculation_type == 2) {
                    cat.totPts += a.rawGrade.pts * a.factor;
                    cat.maxPts += a.rawGrade.max * a.factor;
                }
                else {
                    cat.totPts += a.rawGrade.pts / a.rawGrade.max * a.factor;
                    cat.maxPts += a.factor;
                }
            }
        }
        ...
        for (let i = 0; i < s.grading_category.length; i++) {
            const points = categories[i].div.firstElementChild.getElementsByClassName("categorypoints")[0] ?? document.createElement("div");
            const percent = categories[i].div.firstElementChild.getElementsByClassName("categorypercent")[0] ?? document.createElement("div");
            points.innerText =
            ...
            percent.innerText = roundGrade(categories[i].totPts/categories[i].maxPts * 100);
        ...
        document.getElementById("calculated-grade").innerText = roundGrade(calcGrade * 100);
    }
                ...
                await multiGet(request).then((v) => {
                    for (let i = 0; i < v.response.length; i++) {
                        ...
                        if (res.response_code === 403) {
                            ...
                            if (res.body.factor) {
                                assignments[r[ai].assignmentI].factor = parseFloat(res.body.factor);
