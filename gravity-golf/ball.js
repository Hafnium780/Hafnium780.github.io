class Ball {
	...
	update(dt) {
		...
		for (const planet of map.planets) {
			...
			this.vx +=
				(dt * map.g * map.radiusToMass(planet.r) * (planet.x - this.x)) /
				r ** 3;
			this.vy +=
				(dt * map.g * map.radiusToMass(planet.r) * (planet.y - this.y)) /
				r ** 3;
		}
		this.x += this.vx * dt;
		this.y += this.vy * dt;
	...
	projectPath(dt, vx, vy) {
		...
		while (steps < maxProjectionSteps) {
			...
			for (const i in curPlanets) {
				...
				vx += (dt * map.g * planet.m * (planet.x - x)) / r ** 3;
				vy += (dt * map.g * planet.m * (planet.y - y)) / r ** 3;
			}
			x += vx * dt;
			y += vy * dt;
